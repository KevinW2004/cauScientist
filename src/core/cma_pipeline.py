...
from utils import ConfigManager, visualize_graph, visualize_causal_graph
...
class CMAPipeline:
    def __init__(self, dataset: CausalDataset):
        ...
        # 统一注入 llm_loader 到 hypothesis generator 和 reflection manager
        self.hypothesis_generator = LLMHypothesisGenerator(llm_loader=self.llm_loader)
        self.reflection_manager = ReflectionManager(llm_loader=self.llm_loader)
    ...
    def _save_dataset_info(self):
        ...
        ), "Dataset must be provided to save dataset info."
        info = {
        ...
        info_path = os.path.join(
            self.output_dir, f"{self.domain_name}_dataset_info.json"
        )
    ...
    def run(self) -> None:
        ...
        if initial_graph is None:
            ...
            return
        visualize_graph(initial_graph, filename=f"{self.domain_name}_initial_graph.html", text_only=False, auto_open=True)
        # 初始图评分
        ...
        self.searcher: SearchStrategy = SearcherFactory.create_searcher(
            strategy_name=strategy_name, initial_graph=initial_graph
        )

        print(
            f"Initial hypothesis graph generated. {strategy_name} search strategy initialized."
        )
        print(
        ...
        for t in range(1, num_iterations + 1):
            ...
            # 生成新假设图列表
            new_graphs, is_final_graph = (
                self.hypothesis_generator.generate_next_hypothesis(
                    variable_list=self.variable_list,
                    domain_name=self.domain_name,
                    domain_context=self.domain_context,
            ...
            if is_final_graph:
                print(
                    f"Iteration {t}: LLM indicates current graph is final. Marking and skipping modification."
                )
            ...
            for idx, new_graph in enumerate(new_graphs, 1):
                ...
                print(
                    f"    Score - LL: {fitting_results['cv_log_likelihood']:.4f}, BIC: {fitting_results['bic']:.4f}"
                )
                # 与前图对比：
                # 1. 计算 score_diff 和 is_better 标记, 生成反思.
                # 2. 只有评分提升的图才加入搜索器
                if (
                    new_graph.metadata.log_likelihood is not None
                    and current_graph.metadata.log_likelihood is not None
                ):
                    # 使用 LL， 越大越好, 因此 is_better 判断上升
                    score_diff = (
                        new_graph.metadata.log_likelihood
                        - current_graph.metadata.log_likelihood
                    )
                    is_better = score_diff > 0
                    self.reflection_manager.generate_reflection(
                        self.domain_name,
                        self.domain_context,
                        new_graph,
                        score_diff,
                    ...
                    if not is_better:
                        print(" Rejected. (LL decreased from previous graph).")
                        update_graphs.remove(new_graph)
    ...
    def _evaluate_against_ground_truth(self, predicted_graph: StructuredGraph):
        ...
        assert (
            self.dataset is not None
        ), "Dataset must be provided to evaluate against ground truth"
    ...
    def _generate_final_report(self) -> str:
        ...
        visualize_causal_graph(best_graph, filename=f"{self.domain_name}_best_graph.html")
        lines.extend(["\n" + "-" * 70, "Best Found Structure:"])
